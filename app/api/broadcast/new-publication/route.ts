 ...
 * step, a manual curl) can email the list — not random internet goblins.
 *
...
export async function POST(req: Request) {
  ...
  let body: (PublicationPayload & { sendImmediately?: boolean }) | null = null;
  try {
    body = await req.json();
  } catch {
    return NextResponse.json({ ok: false, error: "Invalid JSON." }, { status: 400 });
  }
  if (!body || (body.type !== "essay" && body.type !== "field_note" && body.type !== "announcement")) {
    return NextResponse.json({ ok: false, error: "type must be essay | field_note | announcement." }, { status: 400 });
  }
