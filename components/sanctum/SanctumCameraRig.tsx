...
const MAX_YAW = 0.42;
const MAX_PITCH = 0.2;
const FREE_YAW = 1.15; // free-roam lets you look much further around
const FREE_PITCH = 0.5;
...
export default function SanctumCameraRig({
  ...
  useFrame((state, dt) => {
    ...
    // breathing when idle, a footstep bob when walking
    const bobY = Math.sin(bobPhase.current) * 0.05 * moveAmt + Math.sin(t * 1.3) * 0.012 * (1 - moveAmt);
    const swayAmt = Math.cos(bobPhase.current * 0.5) * 0.04 * moveAmt;
